...
class UploadFolderRequest(BaseModel):
    """Request model for uploading a folder by path

    Attributes:
        folder_path: The absolute path to the folder to upload
    """

    folder_path: str = Field(
        min_length=1,
        description="The absolute path to the folder to upload",
    )
...
def create_document_routes(
    ...
    async def upload_folder(background_tasks: BackgroundTasks, request: UploadFolderRequest):
        """
        Upload a folder by path to the input directory and index its contents.

        This endpoint accepts a folder path, checks if it exists and is a directory,
        copies it to the input directory, and processes all supported files within it.

        Args:
            background_tasks: FastAPI BackgroundTasks for async processing
            request (UploadFolderRequest): The request body containing the folder path

        Returns:
            InsertResponse: A response object containing the upload status and a message.
                status can be "success", "duplicated", "partial_success", or "failure".

        Raises:
            HTTPException: If the folder doesn't exist (404), isn't a directory (400),
        ...
        try:
            ...
            if not folder_path.is_dir():
                raise HTTPException(
                    status_code=400,
                    detail=f"The path {folder_path} is not a directory"
                )
            
            # Get folder name for destination
            folder_name = folder_path.name
            destination = doc_manager.input_dir / folder_name
            
            # Check if folder already exists in input directory
            if destination.exists():
                return InsertResponse(
                    status="duplicated",
                    message=f"Folder '{folder_name}' already exists in the input directory."
                )
            
            # Copy the folder to input directory
            shutil.copytree(folder_path, destination)
            
            # Scan for files in the newly copied folder
            new_files = []
            for ext in doc_manager.supported_extensions:
                for file_path in destination.rglob(f"*{ext}"):
                    if file_path.is_file():
                        new_files.append(file_path)
            ...
            if not new_files:
                return InsertResponse(
                    status="partial_success",
                    message=f"Folder '{folder_name}' copied successfully, but no supported files were found."
                )
            
            # Process the files
            background_tasks.add_task(pipeline_index_files, rag, new_files)
            
            return InsertResponse(
                status="success",
                message=f"Folder '{folder_name}' uploaded successfully with {len(new_files)} files. Processing will continue in background."
            )
